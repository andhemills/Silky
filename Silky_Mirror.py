...
#
# Geometry note -- handedness / surfaces:
#   A reflection is orientation-reversing: it flips surface normals and the
#   handedness of the (u,v) parametrisation. For multi-row CONTROL GRIDS we
#   correct this automatically -- each row (the u-direction) is reversed after
#   reflection -- so a mirrored grid keeps right-handed (u,v) ordering and a
#   surface fitted to it comes out with a correct, consistent normal. This is
#   unconditional for grids (no prompt, no flag), since a grid is always meant
#   to be surface-able. 1-row control polys have no v direction to preserve
#   and are left untouched.
#   For an actual fitted Shape (objects that carry a real B-rep) we mirror the
#   Shape itself (Part transform with a reflection matrix) so the visible
#   geometry is a true mirror image, including curvature.
#
# ---------------------------------------------------------------------------
# Changelog
# ---------------------------------------------------------------------------
...
#        - Multi-row grids automatically have each row (u-direction) reversed
#          so the mirrored net keeps right-handed (u,v) ordering and stays
#          directly surface-able with a correct normal. No prompt, no flag.
#
# ---------------------------------------------------------------------------
...
def reverse_rows(seq, columns):
    """Reverse the order of elements WITHIN each row of a row-major net.

    A reflection is orientation-reversing: mirroring a control GRID flips the
    handedness of its (u,v) parametrisation, so a surface later fitted to the
    raw mirrored poles comes out with an inverted normal and reversed u
    direction. Reversing each row (the u-direction, `columns` wide) restores
    right-handed (u,v) ordering while leaving the row order (v) intact -- the
    same per-row flip the Silk surface classes apply for their `reverse` flag
    (e.g. CubicSurface_44 maps [0,1,2,3] -> [3,2,1,0] within each band).

    Example, 4 columns:  [r0,r1,r2,r3, r4,r5,r6,r7, ...]
                      ->  [r3,r2,r1,r0, r7,r6,r5,r4, ...]

    Returns a new list; input is not mutated. Only meaningful for multi-row
    grids -- a 1-row poly has no v direction to keep consistent, so reversing
    it would just flip the curve's parameter direction (handled separately /
    not needed), and callers should not apply this to 1-row nets.
    """
    out = []
    rows = int(len(seq) / columns)
    for i in range(rows):
        row = list(seq[i * columns:(i + 1) * columns])
...
class Silky_Mirror:
    ...
    def execute(self, fp):
        ...
        if src_poles:
            ...
            if src_weights:
                weights = list(src_weights)
            else:
                weights = [1.0] * len(mirrored)

            rows, cols = detect_columns(len(mirrored))

            # A reflection flips (u,v) handedness. For multi-row GRIDS we
            # restore right-handed ordering automatically by reversing each
            # row (the u-direction) -- no prompt, no flag. This keeps a
            # mirrored ControlGrid44_2EdgeSegments directly surface-able with a
            # correctly-oriented normal. 1-row polys have no v direction to
            # preserve, so they are left as-is.
            if rows > 1:
                mirrored = reverse_rows(mirrored, cols)
                weights = reverse_rows(weights, cols)
